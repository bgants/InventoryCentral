...
def register_product_routes(app: APIGatewayRestResolver):
    ...
    def get_product(product_id: str):
        table = resources.get_table()  # Lazy-load here
        resources.logger.info(f"Get product ID: {product_id}")
        try:
            ...
            resources.logger.error(f"Error getting product: {str(e)}")
            return {"statusCode": 500, "body": str(e)}

    @app.post("/products")
    def create_product():
        table = resources.get_table()  # Lazy-load here
        resources.logger.info("Create product")
        try:
            ...
            resources.logger.info(f"Inserted: {product_id} at {location_id}")
            return {
                "statusCode": 200,
                "body": {
            ...
            resources.logger.error(f"Error creating product: {str(e)}")
            return {"statusCode": 500, "body": str(e)}
    ...
    def update_product(product_id: str, location_id: str):
        ...
        resources.logger.info(
            f"Update product with ID: {product_id} at location: {location_id}"
        )
        try:
            ...
            resources.logger.info(f"Updated: {product_id} at {location_id}")
            return {
                "statusCode": 200,
                "body": {
            ...
            resources.logger.error(f"Error updating product: {str(e)}")

    @app.delete("/products/<product_id>/<location_id>")
    def delete_product(product_id: str, location_id: str):
        table = resources.get_table()  # Lazy-load here
        resources.logger.info(
            f"Delete product with ID: {product_id} at location: {location_id}"
        )
        try:
            table.delete_item(
                Key={"product_id": product_id, "location_id": location_id}
            )
            resources.logger.info(f"Deleted: {product_id} at {location_id}")
            return {
                "statusCode": 200,
                "body": {
            ...
            resources.logger.error(f"Error deleting product: {str(e)}")
            return {"statusCode": 500, "body": str(e)}
